...
class YOLODataset(Dataset):
    ...
    def get_anno_by_name(self, name):
        ...
        with open(name, "r", encoding="utf8") as f:
            ...
            for line in f.read().split("\n"):
                ...
                if len(line) == 5 or (len(line) == 6 and not self.depth_mode):
                    cx, cy, w, h = [float(k) for k in line[1:5]]
                    x1, y1 = cx - w / 2, cy - h / 2
                    x2, y2 = x1 + w, y1 + h
                    class_id = int(line[0])
                ...
                # Depth Mode
                elif len(line) == 6 and self.depth_mode:
                    cx, cy, w, h, depth = [float(k) for k in line[1:]]
                    x1, y1 = cx - w / 2, cy - h / 2
                    x2, y2 = x1 + w, y1 + h
                    class_id = int(line[0])

                    num_labels += 1
                    msg["annotations"].append([x1, y1, x2, y2, class_id, depth])   # xywh
                    # use coco data wrapper in val or test mode (necessary for pycoco eval later) 
                    if not self.is_train:
                        if not has_anno:
                            self.coco_data.add_image(
                                image_id=self.idx - self.unused_image,
                ...
                elif (len(line) > 1 and len(line) < 5) or len(line) > 6:
                    raise ValueError(f'Unexpected amount of annotations - len(line) < 5 or len(line) > 6 - in file {name}')
    ...
    def _load_yolo_annotations(self):
        ...
        if os.path.isfile(cache_file) and self.use_cache:
            ...
            for self.idx, anno_file in enumerate(tqdm(file_list, unit="images", desc=f"loading {'train' if self.is_train else 'val' if not self.test else 'test'} data")):
