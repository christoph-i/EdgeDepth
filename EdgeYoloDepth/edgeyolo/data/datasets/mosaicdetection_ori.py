...
class MosaicDetection(Dataset):
    ...
    def __init__(
        ...
        mosaic_prob=1.0, mixup_prob=1.0, rank=0, train_mask=True, depth_mode=False, *args
    ):
        """
        Args:
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            if self.depth_mode:
                raise NotImplementedError("Mosaic augmentation used while in depth mode, but this augmentation is not yet implemented for depth mode (because it is most likley counter-productive for depth learning)")
            mosaic_labels = []
            segments = []
            ...
            if (
                ...
                if self.depth_mode:
                    raise NotImplementedError("Mixup augmentation used while in depth mode, but this augmentation is not yet implemented for depth mode (because it is most likley counter-productive for depth learning)")
                if self.train_mask:
