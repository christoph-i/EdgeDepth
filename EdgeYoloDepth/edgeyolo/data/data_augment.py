...
# pads and resizes the input image to fit the target input size while maintaining aspect ratio
# padding pixels filled with value 14
# convert from HWC to CHW order (Height x Width x Channels to Channels x Height x Width) -> typical yolo order
...
class TrainTransform(BaseTransform):
    def __init__(self, max_labels=50, flip_prob=0.5, hsv_prob=1.0, hsv_gain=(0.5, 0.5, 0.5), depth_mode=False):
    ...
    # augmentations (HSV, Flip) apllied, bboxes changed to xywh format (from xyxy), labels returnes as [class, x, y, w, h] or [class, x, y, w, h, depth] depending on depth mode
    def __call__(self, image, targets, input_dim, segmentations=None):
        #TODO auf depth_mode umbaune, sodass auch die depth werte am ende mit zurückgegeben werden 
        boxes = targets[:, :4].copy()
        labels = targets[:, 4].copy()
        if self.depth_mode:
            depths = targets[:, 5].copy()
        if len(boxes) == 0:
            if self.depth_mode:
        ...
        # filter out boxes which have width or height under 1 pixel (only keep boxes with at least 1 pixel width and height )
        mask_b = np.minimum(boxes[:, 2], boxes[:, 3]) > 1
        boxes_t = boxes[mask_b]
        labels_t = labels[mask_b]
        if self.depth_mode:
            depths_t = depths[mask_b]
        segm_reserve = None
        if segmentations is not None:
            if self.depth_mode:
                raise NotImplementedError("Segmentation is activated while in depth mode. Segementation + depth is not (yet) implemented and tested.")
            segm_reserve = []
        ...
        if self.depth_mode:
            depths_t = np.expand_dims(depths_t, 1)
            targets_t = np.hstack((labels_t, boxes_t, depths_t))
