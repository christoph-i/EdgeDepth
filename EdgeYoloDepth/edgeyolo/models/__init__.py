...
def load_model(cfg_file, nc=None, depth_mode=False) -> Model:
    ...
    my_model = Model(cfg_file, nc=nc, is_file=not (isinstance(cfg_file, dict) or not os.path.isfile(cfg_file)), depth_mode=depth_mode)
...
class EdgeYOLO:
    ...
    def __init__(self, cfg_file=None, weights=None, rank=0, write_cfg_to_weights=False, nc=None, depth_mode=False):
        self.rank = rank
        assert cfg_file is not None or weights is not None

        self.__weights = weights
        # use existing weights (e.g. loading model from .pth file)
        if weights is not None and os.path.isfile(weights):
            ...
            # time.sleep(1000)
            self.model = load_model(self.cfg_data, nc=self.ckpt["cfg_data"]["nc"], depth_mode=depth_mode)
            try:
        ...
        # build model from scratch without weights - only based on config file 
            ...
            self.model = load_model(cfg_file, nc=nc, depth_mode=depth_mode)
