import warnings
from .loss import YoloLoss, iou_loss, cls_loss, conf_loss, depth_loss
...
class Trainer(EdgeYOLO):
        ...
        with NoPrint(rank):
            ...
            if params.get("dataset_cfg") is not None:
                with open(params.get("dataset_cfg"), "r") as f:
                    ...
                    nc = len(names)
            self.depth_mode = params["depth_mode"] if params["depth_mode"] else False
            super(Trainer, self).__init__(params["model_cfg"] if "model_cfg" in params else None,
                                          params["weights"],
                                          rank,
                                          params["use_cfg"],
                                          nc=nc, depth_mode=self.depth_mode)
    ...
    def load_init(self):
        ...
        """MOst time consuming part of initial loading"""
        ...
        def load_evaluator():
            ...
            if self.depth_mode and self.dataset_cfg.get("type") != 'yolo':
                raise NotImplementedError(f"Dataset type is not \'yolo\' but depth mode is selected.")
        ...
        def load_loss():
            ...
            if self.depth_mode and not self.params["depth_loss_weight"]:
                warnings.warn("Depth_mode is used but param depth_loss_weight is not set in config file. Default value of 1.0 will be used!")
            self.loss = YoloLoss(
                ...
                depth_loss=depth_loss(self.params["loss_use"][3]) if self.params["depth_mode"] else None,
                depth_loss_weight=self.params["depth_loss_weight"] if self.params["depth_loss_weight"] else 1.0,
    ...
    def train(self):
        ...
        def before_train():
            ...
            # shutil.copyfile(self.params["train_cfg"], os.path.join(self.params["output_dir"], "traing_config.yaml"))
        ...
        def train_one_epoch():
            ...
            def train_one_iter():
                ...
                def train_in_iter():
                    ...
                    # targets format = [bs, max_dets, 1*cls+4*bbox+1*depth]
                    ...
                    inps, targets = preprocess(inps, targets, self.input_size, depth_mode=self.depth_mode)
                    data_end_time = time.time()

                    with torch.cuda.amp.autocast(enabled=self.params["fp16"]):
                        while True:
                            try:
                                # outputs shape = [batchsize, 1,  grid_w, 	grid_h, 	4bbox+1conf+2classifications+1depth]
