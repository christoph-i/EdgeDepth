...
class COCOEvaluator:
    ...
    def __init__(
        self, dataloader, img_size, confthre, nmsthre, num_classes, testdev=False, rank=0, obj_conf_enabled=True, save=False, depth_mode=False
    ):
        """
        Args:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        if self.depth_mode:
                warnings.warn("Attempting coco evaluation in depth mode - tis is not yet tested and implemented completly.")
        ...
        if len(data_dict) > 0:
            # TODO checken ob das hier jemals genutzt wird im yolo case
            if self.depth_mode:
                warnings.warn("Attempting coco evaluation in depth mode - tis is not yet tested and implemented completly.")
