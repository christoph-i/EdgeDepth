...
out_dir = '/leonardo/home/usertrain/a08trb14/distances_pixel/'
...
for i in range(annotations.shape[0]):
  ...
  if detection_result.pose_landmarks:
    ...
    bones_length = []
    for b in bones:
      bones_length.append(pixel_distance(kps[b[0]][0],kps[b[0]][1],kps[b[1]][0],kps[b[1]][1]))
    #kps = np.array(kps).reshape(1,-1)
    row = np.concatenate((np.array(i_prev).reshape((1,1)), np.array(bones_length).reshape((1,-1))),axis=1)
  else:
    row = np.zeros((1,11))
    row[0,0] = i_prev
    row = row.tolist()
